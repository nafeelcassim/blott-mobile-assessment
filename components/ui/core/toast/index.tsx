...
const toastStyle = tva({
  base: "p-4 rounded-md gap-1 web:pointer-events-auto shadow-hard-5 border-outline-100",
  variants: {
    action: {
      error: "bg-error-800",
...
const Toast = React.forwardRef<React.ComponentRef<typeof Root>, IToastProps>(
  function Toast(
    ...
    const maxWidth = 420;
    const computedWidth = Math.min(width - horizontalMargin * 2, maxWidth);

    return (
      <Root
        ref={ref}
        className={toastStyle({ variant, action, class: className })}
        style={[{ width: computedWidth, alignSelf: "center" }, props.style]}
...
function ErrorToast({ nativeID, title, description }: ErrorToastProps) {
  return (
...
export { ErrorToast, Toast, ToastDescription, ToastTitle, useToast };
